...
## 部署
```
[root@master01 local-path-provisioner]# kubectl create ns local-path
[root@master01 local-path-provisioner]# helm install -f values.yaml -n local-path local-path-provisioner .
···
NOTES:
...
[root@master01 local-path-provisioner]# helm ls -n local-path
NAME                    NAMESPACE       REVISION        UPDATED                                 STATUS          CHART                                   APP VERSION
local-path-provisioner  local-path      1               2022-11-29 19:17:58.989130964 +0800 CST deployed        local-path-provisioner-0.0.24-dev       v0.0.24-dev
```
---
# 如下为官方配置解释 
